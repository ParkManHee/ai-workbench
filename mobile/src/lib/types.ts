...
export interface ChatMsg { role: "user" | "assistant"; text: string; tools?: string[]; toolDetails?: string[] }
...
export interface SessionInfo { session_id: string; updated: number; preview: string; count: number; active: boolean; waiting: boolean }
/** 데몬 `/transcript/:project/:sessionId` 응답의 메시지 항목. */
export interface TranscriptMsg { role: string; text: string; tools: string[]; tool_details: string[] }
